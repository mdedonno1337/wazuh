...
void test_wdb_insert_dbsync_bind_fail(void **state)
{
    ...
    expect_string(__wrap__merror, formatted_msg, "DB(global) sqlite3_bind_int(): bad parameter or other API misuse");


    expect_value(__wrap_sqlite3_bind_text, pos, 2);
    expect_string(__wrap_sqlite3_bind_text, buffer, "data");
    will_return_always(__wrap_sqlite3_bind_text, SQLITE_ERROR);
    expect_string(__wrap__merror, formatted_msg, "DB(global) sqlite3_bind_text(): bad parameter or other API misuse");
